...
function drawText(context, text, font, x, y, size, spacing, color, align, height) {
...
function drawBackground(context, dpiVal, bleed) {
    ...
    if (customImage && imageLayer === 'background') {
        ...
        const customImageScale = parseFloat($('customImageScale')?.value || 1);
...
function drawArea9(context, dpiVal, bleed) {
    ...
    const text10Size = parseFloat($('text10Size')?.value || 7MOR) * dpiVal / 72;
    ...
    drawText(context, text10, font, text10X + (bleed ? sizes.ble latencyVal / 25.4 : 0), text10Y + (bleed ? sizes.bleed * dpiVal / 25.4 : 0), text10Size, text10Spacing, footerTextColor);
...
function loadFont(file, fontKey) {
    ...
    reader.onload = e => {
        ...
        font.load().then(loadedFont => {
            ...
            console.error(langs[currentLang].fontLoadError);
            debouncedDrawTicket();
        });
    };
